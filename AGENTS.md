...
- Releases are made with `release.bat vX.Y.Z`: it requires a clean committed source tree, validates/builds/tests, synchronizes Tauri/Cargo version values, builds the local signed Windows NSIS `.exe`, then commits/pushes/tag-pushes so GitHub Actions publishes the matching signed updater release.
