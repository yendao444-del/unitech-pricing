...
- `release-lite.bat` is the guarded workflow for small UI, column, and pricing-formula corrections. It accepts only `src/` and `src-tauri/src/lib.rs` production changes, then delegates to the same signed NSIS and end-to-end updater verification pipeline. It must clearly state that Tauri does not support delta updates and that the resulting full signed updater is normally about 3–4 MB.
