...
- Releases are normally made by double-clicking `release.bat`: it automatically increments the patch version, requires a clean committed source tree, validates/builds/tests, synchronizes Tauri/Cargo version values, builds the local signed Windows NSIS `.exe`, then commits/pushes/tag-pushes so GitHub Actions publishes the matching signed updater release. An explicit `vX.Y.Z` argument is only for intentional minor/major versions.
